...
class Day16: Day {
    ...
    override func resolveA<T>(_ input: [String]) -> T {
        let processedInput = input.dropLast()
        let pipes = Pipes(valves: processedInput.map({ Valve(line: $0) }))
    ...
    private func findAllPaths<Node>(from start: Node, using getNextNodes: (Node) -> [Node]) -> [[Node]] {
        var paths: [[Node]] = []
...
func findShortestPath<Node: Hashable>(from start: Node, using getNextNodes: ((Node) -> [Node]?)) -> [Node] {
    ...
    fatalError("No possible path exists")
}
